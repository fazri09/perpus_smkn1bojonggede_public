function executeExample(element, t) {
  switch (t) {
    case "basicMessage":
    ...
    case "kembalikanbukuRequest":
      const idPinjam = element.getAttribute("data-id");
      const nis = element.getAttribute("data-nis");
      const nama = element.getAttribute("data-nama");
      const kode = element.getAttribute("data-kode");
      const buku = element.getAttribute("data-buku");
      ...
      Swal.fire({
        ...
        html: `
          ...
          <p><strong>Kode Buku:</strong> ${kode}</p>
          <p><strong>Judul Buku:</strong> ${buku}</p>
          <input type="date" id="tgl_kembali" class="swal2-input" value="${today}" style="width: 70%;">
        `,
        focusConfirm: false,
        showCancelButton: true,
        confirmButtonText: "Kembalikan",
        preConfirm: () => {
          const tanggal = document.getElementById("tgl_kembali").value;
          if (!tanggal) {
            Swal.showValidationMessage("Tanggal tidak boleh kosong");
            return false;
          }
          return fetch(base_url + "pinjaman/pinjaman_kembali", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: idPinjam, tanggal_kembali: tanggal })
          })
          .then(response => {
        ...
        if (result.isConfirmed) {
          Swal.fire({
            ...
            location.reload(); // pakai ini kalau ingin reload halaman
          });
        }
      });
    case "chainingModals":
